// app/search/page.tsx — Search (SSR, dark/light, bots + blogs, đồng bộ phong cách)
...
export default async function SearchPage({
  ...
  const q = (sp.q || "").trim();
  const tab = (sp.tab || "blogs").toLowerCase();
  const pageNum = Math.max(1, parseInt(String(sp.page || "1"), 10));
  const current = { q, tab, page: pageNum };

  // dữ liệu
  let blogs: SearchBlogItem[] = [];
  let bots: SearchBotItem[] = [];
  let countBlogs = 0;
  let countBots = 0;

  if (q) {
    // Luôn fetch ở chế độ "all" → counts cố định
    const resp = await SearchApi.all(q, {
      // phân trang tùy theo tab đang xem
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {[
              ...
              return (
                <Link
                  key={t.key}
                  href={withQuery("/search", current, { tab: t.key, page: 1 })}
          ...
          <>
            ...
            {(tab === "bots" || tab === "all") && (
              <>
                <div className="mb-3 flex items-end justify-between">
                  ...
                  {tab === "all" && (
                    <Link
                      href={withQuery("/search", current, { tab: "bots", page: 1 })}
                ...
                {bots.length === 0 ? (
                  ...
                  <div className="mb-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {bots.map((b) => {
                      const text = b.summary || b.description || b.name || "";
            ...
            {(tab === "blogs" || tab === "all") && (
              <>
                <div className="mb-3 flex items-end justify-between">
                  ...
                  {tab === "all" && (
                    <Link
                      href={withQuery("/search", current, { tab: "blogs", page: 1 })}
                      className="text-sm text-neutral-600 underline-offset-4 hover:underline dark:text-neutral-300"
                    >
                      Xem tất cả blogs →
                    </Link>
                ...
                {blogs.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {blogs.map((b) => {
                      const href = `/blogs/${encodeURIComponent(b.slug)}`; // slug-only
                      const text = toPlain(b.snippet || "");
                      const snippet = text ? b.snippet : ""; // backend đã highlight; nếu muốn tự làm: makeSnippet(...)
                      return (
                        <article
                          key={b._id}
                          className="group overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm transition hover:shadow-lg dark:border-white/10 dark:bg-white/5"
            ...
            {(tab === "blogs" ? countBlogs : tab === "bots" ? countBots : Math.max(countBlogs, countBots)) >
              ...
              <div className="mt-10 flex items-center justify-center gap-2">
                <Link
                  href={withQuery("/search", current, { page: Math.max(1, pageNum - 1) })}
                ...
                <Link
                  href={withQuery("/search", current, { page: pageNum + 1 })}
